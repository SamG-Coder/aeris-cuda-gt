...
export async function runDeviceChecks(runtime,source){
 ...
 try{
  ...
  const drive=async(steps,values={},stabilityControl=1)=>{const input={drive:0,brake:0,steering:0,handbrake:0,shift:0,...values};for(let n=0;n<steps;n+=8){const batch=runtime.batch();recordDriving(batch,b,input,{automatic:1,tractionControl:1,stabilityControl,wetness:0,showroom:0},Math.min(8,steps-n));batch.dispatch(b.pose,[1]).submit();}await runtime.idle();};
